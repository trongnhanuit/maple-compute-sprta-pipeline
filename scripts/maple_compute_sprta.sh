...
MAPLE_PARAMS="--SPRTA --overwrite --networkOutput --keepInputIQtreeSupports --doNotImproveTopology " # MAPLE params
...
for aln_path in "${ALN_DIR}"/*.maple; do
    ...
    echo "cd ${ALN_DIR} && ${PYPY_PATH} ${MAPLE_PATH} --input ${aln} --output ${ALN_DIR}/${MAPLE_SPRTA_TREE_PREFIX}${aln} --inputTree ${TREE_DIR}/${CMAPLE_SPRTA_TREE_PREFIX}${aln}.treefile.nexus --model ${MODEL} ${MAPLE_PARAMS} ${BL_FIXED_OPT} ${NOT_REROOT_OPT} ${ZERO_LENGTH_BRANCHES_OPT}"
    cd ${ALN_DIR} && ${PYPY_PATH} ${MAPLE_PATH} --input ${aln} --output ${ALN_DIR}/${MAPLE_SPRTA_TREE_PREFIX}${aln} --inputTree ${TREE_DIR}/${CMAPLE_SPRTA_TREE_PREFIX}${aln}.treefile.nexus --model ${MODEL} ${MAPLE_PARAMS} ${BL_FIXED_OPT} ${NOT_REROOT_OPT} ${ZERO_LENGTH_BRANCHES_OPT}
